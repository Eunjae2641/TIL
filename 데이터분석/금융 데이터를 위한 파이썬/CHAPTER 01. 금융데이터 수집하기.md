...
웹 크롤링을 이용하면 인터넷에 공개된 데이터를 자동화하여 수집할 수 있다.
이를 위해서는 인터넷에서 데이터를 송수신하는 방식과 웹 페이지 언어인 HTML, CSS, JAVASCRIPT에 대해 기초 지식을 알아 두는 것이 좋다.

## 1.3.1 인터넷 데이터 송수신 방식

우리가 웹 브라우저로 URL을 입력하면, 입력한 URL의 웹 서버에 웹 페이지 정보를 요청한다. 
웹 서버는 요청을 받아서 웹 페이지 정보를 요청한 곳에 회신한다.
회신된 웹 페이지 정보는 HTML이란 언어로 되어 있는데 웹 브라우저가 이를 분석하여 우리 눈에 보이는 웹 페이지로 보여준다.

인터넷에서 데이터를 송수신하는 방법은 이렇게 요청하고 응답하는 구조로 되어 있다.

웹 브라우저에 있는 개발자 도구를 통해 HTML 언어를 확인할 수 있다.

크롬 브라우저 메뉴의 개발자 도구를 선택하면 화면 우측에서 지금 페이지의 HTML 언어를 볼 수 있다.

웹 서버에 웹 정보를 요청하는 방법은 여러 가지가 있다. 

**GET 방식**
우리가 웹 서버에 웹 페이지를 요청할 때는 모두가 동일한 페이지를 요청할 수도 있지만, 검색과 같이 자신에게만 필요한 웹 페이지 정보를 요청할 수 도 있다.
GET 방식은 URL에 파라미터를 넣어 전송하는 방법으로 API를 이용하여 데이터를 요청할 때 처럼 URL에 필요한 정보를 넣어 전송한다.
방식은 간편하나 너무 긴 텍스트는 보내기 어렵고 전송 데이터가 URL에 노출된다는 단점이 있다.
인터넷 검색도 이 방식으로 데이터를 송수신한다.

**POST 방식**
POST 방식은 URL에 포함되었던 정보를 요청 메시지 안에 넣어서 전송하는 방식이다.
GET 방식과 달리 긴 텍스트도 보낼 수 있고 전송 데이터가 URL에 노출되지 않는다.
POST 방식을 확인하려면 크롬의 개발자 도구 화면에서 NETWORK 탬을 이용해야한다. 여기서는 브라우저와 서버 간의 통신 과정을 살펴볼 수 있다.
NETWORK 탭 하단의 METHOD가 POST 항목을 찾으면 된다. 해당 항목을 클릭하면, 우측 하단의 FormData에서 데이터를 요청한 내역을 확인할 수 있다.

## 1.3.2 HTML(Hyper Text Markup Language) 알아보기
HTML의 문서 간 이동이 자유로운 태그로 이루어진 언어이다. 문서의 서식이나 필요한 기능들은 태그라는 것을 이용하여 나타낸다.
태그는 <문자> ~ <문자>로 이루어져 있다. 
이미지를 넣을 때는 문자에 해당하는 부분에 image의 약자인 img를, 문자를 굵게 하고 싶을 때는 bold의 약자인 b를 넣어 사용한다.
원하는 데이터를 추출하기 위해 이 태그 정보를 이용하기에 HTML 태그를 조금 아는 것이 도움된다.

**HTML 문서 기본 구조**
HTML 문서는 <!doctype>과 <html>,<head>,<body>의 네 가지 태그로 문서의 시작과 끝을 표시한다.

<!doctype>은 현재 문서의 종류를 나타내는 태그로, <!doctype html>은 HTML5 언어로 작성된 웹문서를 의미한다,
<html>~</html>은 웹 페이지의 시작과 끝을 나타낸다.
<head>~</head>는 웹 브라우저가 웹 페이지를 해석하기 위해 필요한 정보들이 있는 영역이다.
여기 있는 내용은 제목만 브라우저 창에 나타나고 나머지는 표시되지 않는다.
<body>~</body>는 본문에 해당하는 내용이다. 실제 웹 브라우저에 나타나는 내용들이 있다.
관련 코드는 code 폴더의 test.html을 참고.

**태그의 구조**
HTML 태그는 태그 이름과 속성, 내용으로 이루어진다. 웹 페이지를 크롤링한 후 필요한 부분을 추출할 때는 이 태그나 속성, 내용을 가지고 찾아낸다.

<문자>로 묶인 부분이 HTML 태그이다. 대부분의 태그가 여는 태그와 닫는 태그의 쌍으로 이루어져 있다.
닫는 태그는 </문자>로 작성하는데, 여는 태그와 닫는 태그 사이를 적용 범위로 인식하다.
<img>나 <br>처럼 닫는 태그가 필요 없는 것도 일부 있다.

속성은 태그에 여러 가지 기능을 추가하기 위해 입력한다. <태그 이름 속성1="속성값" 속성2="속성값" ···>의 형태로 입력한다.
예를 들어, 글자의 폰트를 지정하려하면 font 태그를 사용한다. 이 태그를 통해 폰트의 크기나 색깔 등을 속성으로 입력할 수 있다.
태그마다 사용할 수 있는 속성은 모두 다르다.

내용은 웹 브라우저에서 우리에게 보이는 텍스트이다.

HTML 코드로 '파이썬으로 금융데이터 수집하기'를 글자 크기 10, 글자색은 파란색으로 작성한다면, 아래와 같이 작성하면 된다.
```<font size="10" color="blue">파이썬으로 금융 데이터 수집하기</font>```

**텍스트를 처리하는 태그 - <h 숫자>,<p>,<br>**
<h 숫자> 태그는 HTML 문서 안에서 제목을 나타낼 때 사용되는 태그이다.
다른 텍스트보다 진하고 크게 표시되며 크기에 따라 <h1>~<h6>까지 사용할 수 있다.
h1이 가장 크고 h6이 가장 작다.

```<h1> 이것은 제목입니다.</h1>```

<p> 태그는 문서에 단락을 나눌 때 사용하는 태그이다. <p> 태그의 앞뒤로 줄 바꿈이 일어난다.
텍스트를 표시할 때 가장 많이 사용되는 태그이며 <p> 태그와 함께 속성을 입력하여 단락별로 서식을 지정할 수 있다.

```<p>문서의 단락입니다.</p>```

<br>태그는 줄 바꿈을 하는 태그이다. 줄을 바꾸고 싶은 위치에 <br> 태그를 입력하면 된다.
<br>태그는 닫는 태그가 없다.
